...
```
# 10 line 10 print invader with sound, top down, color, bomb evasion, bug fix past bottom of screen -- 12 char per line version
```basic
0 x=20:y=2:m=-1:n=-1:i=0:j=22:d=1:s=1024:c=55296:js=56321:h=205.5:t=77.5:u=71.5:poke53280,0:poke53281,0:print"{clear}";
1 a=y*40+x:pokes+a,32:x=x+(peek(js)and4)/4*1-(peek(js)and8)/8:x=x-40*int(x/40):y=y+(peek(js)and1)-(peek(js)and2)/2:ify<2theny=2:ify>12theny=12
2 pokec+j*40+i,14:pokes+j*40+i,t+rnd(1):i=i+d:ifpeek(s+j*40+i)=42ori>38ori<1thend=-d:j=j-1
3 a=j*40+i:pokec+a,iand15:pokes+a,h+rnd(1):ifj<yand(i>38ori<1)thenv=8:f=128:g=-2:gosub9:j=22:i=0:d=1

...
```basic
0 x=20:y=2:l=l+1:m=-1:n=-1:i=0:j=21-l:d=l:s=1024:c=55296:js=56321:h=205.5:t=77.5:u=71.5:poke53280,0:poke53281,0:print"{clear}"
...
5 if(peek(js)and16)=0andm<0thenm=x:n=y+1:v=5:f=64:g=-4:gosub9
6 ifm>=0thenpokes+n*40+m,32:n=n+1:a=(nand31)*40+m:o=nand15:pokec+a,o:pokes+a,u+rnd(1):ifn=jandm<>ithenpokec+a,o:pokes+a,42:m=-1
...
```
## single line keyboard movement (l,r:o,p & u,d:e,d)
```basic
1 pokes+y*40+x,32:k=peek(197):x=x-(k=39)+(k=40):y=y-(k=14)+(k=18)
```
# 10 line 10 print invader with sound, upwards, color, bomb evasion, bug fix past bottom of screen -- 120 char per line version
```basic
...
1 pokes+y*40+x,32:x=x+(peek(js)and4)/4-(peek(js)and8)/8:x=x-40*int(x/40):y=(y+(peek(js)and1)-(peek(js)and2)/2)and15
2 pokec+j*40+i,14:pokes+j*40+i,t+rnd(1):i=i+d:ifpeek(s+j*40+i)=42ori>38ori<1thend=-d:j=j-1
3 a=j*40+i:pokec+a,iand15:pokes+a,h+rnd(1):ifj<yand(i>38ori<1)thenv=8:f=128:g=-2:gosub9:j=21-l:i=0:d=1
4 b=y*40+x:pokec+b,3:pokes+b,83:ify>=jor(j=yandi=x)thenprint"{home}{down}{down}{yel}boom!{lblu}":v=15:f=255:g=-4:gosub9:end
...
9 poke54296,v:poke54277,0:poke54278,240:poke54276,16:poke54276,129:fore=fto0stepg:poke54273,e:next:poke54276,128:return
```
# 10 line 10 print invader with sound, upwards, color, bomb evasion, buggy -- 120 char per line version
```basic
0 x=20:y=2:l=l+1:m=-1:n=-1:i=0:j=21-l:d=1:s=1024:c=55296:js=56321:h=205.5:t=77.5:u=71.5:poke53280,0:poke53281,0:print"{clear}"
...
```
# 10 line 10 print invader with sound, color, bomb evasion, bug fix past bottom of screen -- 120 char per line version
```basic
